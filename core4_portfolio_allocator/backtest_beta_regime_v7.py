"""V5 breakout with one causal BTC beta-regime direction filter."""
...
REPORT_ROOT = ROOT / "reports" / "beta_regime_v7"
...
def beta_regime_breakout_side(
    markets: dict[str, pd.DataFrame], symbol: str, date: pd.Timestamp
) -> str | None:
    side = breakout_side(markets, symbol, date)
    btc = markets["BTCUSDT"]
    if side is None or date not in btc.index:
        return None
    row = btc.loc[date]
    if pd.isna(row["sma200"]):
        return None
    if side == "LONG" and row["close"] > row["sma200"]:
        return side
    if side == "SHORT" and row["close"] < row["sma200"]:
        return side
    return None
...
def main() -> int:
    ...
    for year in range(FIRST_OOS_YEAR, int(last_date.year) + 1):
        ...
        path, trades, diagnostics = simulate(
            ...
            STRESS_COST,
            start,
            entry_side_function=beta_regime_breakout_side,
        ...
        paths.append(path)
        fold_rows.append({"year": year, **path_metrics(path), **trade_metrics(trades), **diagnostics})
    ...
    verdict = {
        "experiment": "CORE4_CLEANROOM_BETA_REGIME_BREAKOUT_V7",
        ...
        "checks": checks,
        "decision": "SECOND_AUDIT_REQUIRED" if all(checks.values()) else "REJECT_V7",
